...
def filter_task(f, whitelist_dict, foutpath, key_name):
    ...
    with open(f, errors='ignore') as fin:
...
def main():
    ...
    #     if args.recursive:
    #         results = [pool.apply_async(filter_task, (f,)+(whitelist, foutpath, key_name)) for f in glob.glob   (finpath+"/**/*.xml", recursive=True)]
    #     else:
    #         results = [pool.apply_async(filter_task, (f,)+(whitelist, foutpath, key_name)) for f in glob.glob   (finpath+"/*.xml")]
    # else:
